    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (error.response?.data?.blocked) {
                ...
                toast.error(`🤖 AI is busy with other users. Please wait ${retryAfter} seconds and try again!`, {
                    duration: 6000,
                    icon: '⏳'
                });
                ...
                setConversation(prev => [
                    ...
                    {
                        role: 'assistant',
                        content: `⏳ Our AI is currently handling many requests. Please wait ${retryAfter} seconds and try again! We support up to 10 concurrent users.`,
                        isError: true
                    }
                ]);
            } else if (error.response?.status === 503) {
                // Service unavailable
                toast.error('🤖 AI service temporarily unavailable. Trying again...', {
                    duration: 4000,
                    icon: '🔄'
                });
